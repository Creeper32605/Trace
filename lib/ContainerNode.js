...
class ContainerNode extends TransformableNode {
	...
	addItem(item) {
		if (!(item instanceof ObjectNode)) throw new Error('Item is not an ObjectNode');
		if (item.parent) throw new Error('Item already has a parent node');
		this._p.items.push(item);
		item.parent = this;
	}
	removeItem(item) {
		if (this.items.includes(item)) {
			this._p.items.splice(this.items.indexOf(item));
